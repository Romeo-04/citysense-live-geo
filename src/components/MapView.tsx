...
const MapView = ({ center, zoom, activeLayers, selectedDate, selectedCity }: MapViewProps) => {
  ...
  const demoLayersRef = useRef<Record<string, { layer: L.Layer; city: string; openedPopup?: boolean }>>({});
        ...
        if (shouldRebuild) {
          (async () => {
            try {
              ...
              demoLayer.addTo(map);
              demoLayer.bringToFront();

              const existing = demoLayersRef.current[layerKey];
              const hasOpened = existing?.openedPopup;
              if (!hasOpened) {
                const firstFeatureLayer = demoLayer.getLayers()[0] as (L.Layer & {
                  openPopup?: () => void;
                }) | undefined;
                if (firstFeatureLayer?.openPopup) {
                  setTimeout(() => {
                    // guard in case layer was removed before timeout fires
                    if (!disposed && map.hasLayer(demoLayer)) {
                      firstFeatureLayer.openPopup();
                    }
                  }, 50);
                }
              }

              demoLayersRef.current[layerKey] = { layer: demoLayer, city: selectedCity, openedPopup: true };
            } catch (error) {
          ...
          existingDemo.layer.bringToFront?.();
        }
      }
    });
